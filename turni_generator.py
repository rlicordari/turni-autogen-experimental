...
# E/G weekly blocks (Mon-Sat) if block_days=6
    ...
    if isinstance(cdiag, dict):
        stats.update(cdiag)
    # Conteggi utili a debug (mese corrente)
    try:
        def _cnt(col: str) -> int:
            return sum(1 for s in slots if (col in [str(c).strip().upper() for c in (s.columns or [])]) and assignment.get(s.slot_id) == "Recupero")
        stats["recupero_T_count"] = _cnt("T")
        stats["recupero_Q_count"] = _cnt("Q")
        # Y affiancamento (slot dedicati Y_REC)
        stats["recupero_Y_affianc_count"] = sum(1 for s in slots if getattr(s, "rule_tag", "") == "Y_REC" and assignment.get(s.slot_id) == "Recupero")
    except Exception:
        pass
...
def write_output(
    ...
    for s in slots:
        ...
        if not (getattr(s, "empty_domain", False) or ((not s.required) and bp > 0)):
            continue
...
def write_solver_log(out_path: Path, stats: Dict) -> Optional[Path]:
    ...
    try:
        ...
        for mk in sorted(months.keys()):
            ...
            # Debug conteggi Recupero (se presenti)
            if any(k in sm for k in ("recupero_T_count","recupero_Q_count","recupero_Y_affianc_count")):
                lines.append(
                    f"recupero: T={sm.get('recupero_T_count')} | Q={sm.get('recupero_Q_count')} | Y_affianc={sm.get('recupero_Y_affianc_count')}"
                )
            if sm.get("autorelax"):
