...
def solve_with_ortools(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    # Reperibilità balancing and spacing (C)
    # Implements:
    # - spacing_min_days (HARD): no consecutive/too-close C assignments for the same doctor
    # - target_per_doctor (SOFT): prefer ~target occurrences per doctor
    # - max_per_doctor (SOFT by default, HARD if max_per_doctor_hard=true)
    # - spacing_preferred_days (SOFT): prefer to keep at least ~N days between C assignments
    if "rules" in cfg and "C_reperibilita" in cfg["rules"]:
        rC3 = cfg["rules"].get("C_reperibilita") or {}
        try:
            c_target = int(rC3.get("target_per_doctor", 0) or 0)
        except Exception:
            c_target = 0
        try:
            c_max = int(rC3.get("max_per_doctor", 0) or 0)
        except Exception:
            c_max = 0
        try:
            c_min_gap = int(rC3.get("spacing_min_days", 0) or 0)
        except Exception:
            c_min_gap = 0
        try:
            c_pref_gap = int(rC3.get("spacing_preferred_days", 0) or 0)
        except Exception:
            c_pref_gap = 0
        c_max_hard = bool(rC3.get("max_per_doctor_hard", False))
        # Penalties (tunable)
        pen_dev = int(rC3.get("penalty_deviation", 30) or 30)
        pen_excess = int(rC3.get("penalty_excess", 200) or 200)
        pen_pref = int(rC3.get("penalty_spacing_preferred", 20) or 20)

        # Collect C vars by (date,doc)
        c_var_by_day_doc = {}
        for s in slots:
            if s.columns == ["C"]:
                for doc in doctors:
                    if (s.slot_id, doc) in x:
                        c_var_by_day_doc[(s.day.date, doc)] = x[(s.slot_id, doc)]

        # HARD spacing: no 2 C within c_min_gap days (gap in days between assignments must be >= c_min_gap)
        if c_min_gap and c_min_gap > 1:
            for doc in doctors:
                for i, drow in enumerate(days):
                    v1 = c_var_by_day_doc.get((drow.date, doc))
                    if v1 is None:
                        continue
                    for k in range(1, c_min_gap):
                        j = i + k
                        if j >= len(days):
                            break
        ...
        # SOFT preferred spacing: penalize pairs too close (< c_pref_gap)
        if c_pref_gap and c_pref_gap > 1:
            for doc in doctors:
                for i, drow in enumerate(days):
                    v1 = c_var_by_day_doc.get((drow.date, doc))
                    if v1 is None:
                        continue
                    for k in range(1, c_pref_gap):
                        j = i + k
                        if j >= len(days):
                            break
                        v2 = c_var_by_day_doc.get((days[j].date, doc))
                        if v2 is None:
                            continue
                        y = model.NewBoolVar(f"c_close_{hash(doc)%10**6}_{drow.date.isoformat()}_{k}")
                        model.Add(y <= v1)
                        model.Add(y <= v2)
                        model.Add(y >= v1 + v2 - 1)
                        extra_obj.append(y * pen_pref)

        # Balance counts per doctor (SOFT) and max cap (soft/hard)
        # Apply only to doctors that can actually appear in C variables
        docs_with_c = {doc for (_, doc) in c_var_by_day_doc.keys()}
        if docs_with_c:
            total_days = len(days)
            for doc in sorted(docs_with_c, key=lambda s: s.lower()):
                cvars = [c_var_by_day_doc[(d.date, doc)] for d in days if (d.date, doc) in c_var_by_day_doc]
                if not cvars:
                    continue
                ccount = model.NewIntVar(0, total_days, f"c_count_{hash(doc)%10**6}")
                model.Add(ccount == sum(cvars))
                if c_max and c_max > 0:
                    if c_max_hard:
                        model.Add(ccount <= c_max)
                    else:
                        excess = model.NewIntVar(0, total_days, f"c_excess_{hash(doc)%10**6}")
                        model.Add(excess >= ccount - c_max)
                        model.Add(excess >= 0)
                        extra_obj.append(excess * pen_excess)
                if c_target and c_target > 0:
                    dev = model.NewIntVar(0, total_days, f"c_dev_{hash(doc)%10**6}")
                    model.AddAbsEquality(dev, ccount - c_target)
                    extra_obj.append(dev * pen_dev)
...
def solve_greedy(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    - respect key HARD constraints (night spacing, night-off-next-day, K no-consecutive if enabled)
    - for C (Reperibilità): avoid consecutive/too-close assignments and balance towards target/max
    """
    ...
    reperC_by_doc: Dict[str, List[dt.date]] = defaultdict(list)
    ...
    # --- C (Reperibilità) parameters
    rC = rules.get("C_reperibilita", {}) if isinstance(rules.get("C_reperibilita"), dict) else {}
    c_target = int(rC.get("target_per_doctor", 0) or 0)
    c_max = int(rC.get("max_per_doctor", 0) or 0)
    c_min_gap = int(rC.get("spacing_min_days", 0) or 0)
    c_pref_gap = int(rC.get("spacing_preferred_days", 0) or 0)
    c_max_hard = bool(rC.get("max_per_doctor_hard", False))
    c_constraints = set(rC.get("constraints") or [])

    # --- Hard quotas / caps from other rules
    ...
    def _has_night_on(date: dt.date, doc: str) -> bool:
        return assignment.get(f"{date}-J") == doc
    ...
    def can_assign(s: Slot, doc: str) -> bool:
        ...
        # C (Reperibilità) spacing + optional coupling with nights
        if s.columns == ["C"]:
            if c_min_gap and c_min_gap > 1:
                for prev in reperC_by_doc[doc]:
                    if abs((s.day.date - prev).days) < c_min_gap:
                        return False
            if c_max_hard and c_max and len(reperC_by_doc[doc]) >= c_max:
                return False
            if "not_night_same_day" in c_constraints:
                if _has_night_on(s.day.date, doc):
                    return False
            if "not_night_next_2_days" in c_constraints:
                for off in (0, 1, 2):
                    d2 = s.day.date + dt.timedelta(days=off)
                    if _has_night_on(d2, doc):
                        return False
    ...
    def score_candidate(s: Slot, doc: str) -> Tuple:
        ...
        if s.columns == ["C"]:
            c_cnt = len(reperC_by_doc[doc])
            excess = max(0, (c_cnt + 1) - c_max) if c_max else 0
            dev = abs((c_cnt + 1) - c_target) if c_target else c_cnt
            last_gap = 999
            if reperC_by_doc[doc]:
                last_gap = abs((s.day.date - reperC_by_doc[doc][-1]).days)
            pref_pen = 0
            if c_pref_gap and c_pref_gap > 1 and last_gap < c_pref_gap:
                pref_pen = (c_pref_gap - last_gap)
            return (excess, dev, pref_pen, c_cnt, -last_gap, load_total[doc], doc.lower())
    ...
    for s in slots_sorted:
        ...
        if s.columns == ["C"]:
            reperC_by_doc[chosen].append(s.day.date)
    ...
    stats = {
        ...
        "C_reper_per_doc": {k: len(v) for k, v in reperC_by_doc.items()},
...
# -------------------------
# Carryover utilities (cross-month constraints support)
# -------------------------

__version__ = "2026-01-20.1"


def extract_carryover_from_output_xlsx(
    xlsx_source,
    sheet_name: str | None = None,
    night_header: str = "Notte",
    date_col_hint: int | None = 1,
    night_col_hint: int | str | None = None,
    lookback_nights: int = 6,
) -> dict:
    """Extract carryover information from a previously generated output XLSX.

    This is meant to support *cross-month* constraints such as:
      - a doctor who worked the NIGHT on the last day of month M should not be scheduled on day 1 of month M+1
      - night spacing constraints at the beginning of a month should consider the last few nights of the previous month

    Parameters
    ----------
    xlsx_source:
        Path/str to an .xlsx file OR bytes-like content (e.g., uploaded file bytes).
    sheet_name:
        Worksheet name. If None, the first sheet is used.
    night_header:
        Header label to identify the night column (case-insensitive match).
    date_col_hint:
        1-based index of the date column if known (default 1). If it doesn't look like dates, an auto-detect is attempted.
    night_col_hint:
        Optional override for the night column. Can be 1-based int index or Excel letter (e.g., "J").
    lookback_nights:
        How many last rows to look back to collect previous nights (default 6).

    Returns
    -------
    dict with keys:
      - sheet
      - last_date (ISO string)
      - last_night_doctor
      - recent_nights: list of {date, doctor}

    Notes
    -----
    - If the last day has no night assigned (e.g., blank night days), last_night_doctor may be None.
    - Names are normalized with `norm_name`.
    """
    import io
    ...
    # Load workbook
    if isinstance(xlsx_source, (str, Path)):
        wb = load_workbook(str(xlsx_source), data_only=True)
    else:
        # bytes-like
        wb = load_workbook(io.BytesIO(xlsx_source), data_only=True)
    ...
    # --- detect date column (1-based)
    def _cell_is_date(v):
        import datetime as _dt
        return isinstance(v, (_dt.date, _dt.datetime))

    date_col = int(date_col_hint) if date_col_hint else 1
    # Validate hint
    v_hint = ws.cell(row=2, column=date_col).value
    if not _cell_is_date(v_hint):
        # auto-detect within first 5 columns
        found = None
        for c in range(1, 6):
            if _cell_is_date(ws.cell(row=2, column=c).value):
                found = c
                break
        date_col = found or date_col

    # --- detect night column (1-based)
    if night_col_hint is not None:
        if isinstance(night_col_hint, int):
            night_col = night_col_hint
        else:
            night_col = column_index_from_string(str(night_col_hint).strip())
    else:
        night_col = None
        ...
        for c in range(1, ws.max_column + 1):
            ...
            if hs == str(night_header).strip().lower():
                night_col = c
                break
        # common fallback: column J if present
        if night_col is None and ws.max_column >= 10:
            night_col = 10
    ...
    if not rows:
        return {
            "sheet": ws.title,
            "last_date": None,
            "last_night_doctor": None,
            "recent_nights": [],
        }

    last_row, last_date = rows[-1]
    last_doc = ws.cell(row=last_row, column=night_col).value if night_col else None
    last_doc = norm_name(last_doc) if last_doc else None

    # recent nights (lookback)
    start_idx = max(0, len(rows) - max(1, int(lookback_nights)))
    recent = []
    for rr, dd in rows[start_idx:]:
        doc = ws.cell(row=rr, column=night_col).value if night_col else None
        doc = norm_name(doc) if doc else None
        if doc:
            recent.append({"date": dd.isoformat(), "doctor": doc})

    return {
        "sheet": ws.title,
        "last_date": last_date.isoformat(),
        "last_night_doctor": last_doc,
