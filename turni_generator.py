...
__version__ = "2026-01-22-strictC-1"
...
def slots_for_month(cfg: dict, days: List[DayRow], unav: Dict[str, Dict[dt.date, Set[str]]]) -> List[Slot]:
    ...
    for s in slots:
        if s.required and not s.allowed:
            # Instead of aborting generation, force this slot to be optional and leave it blank.
            # This happens when a slot has a very small pool (even 1 doctor) and that doctor
            # is unavailable for the day. The admin can fill these cells manually later.
            s.required = False
            s.blank_penalty = 0
            setattr(s, "forced_blank_reason", "empty_allowed_domain")
        if not s.allowed:
...
def solve_across_months(
    ...
    for (yy, mm) in month_keys:
        ...
        forced_blank_slots_m = [s.slot_id for s in slots_m if getattr(s, "forced_blank_reason", None)]
        ...
        # Report slots forced blank due to empty domain
        if forced_blank_slots_m:
            stats_m = dict(stats_m or {})
            stats_m.setdefault("forced_blank_slots", []).extend(forced_blank_slots_m)
