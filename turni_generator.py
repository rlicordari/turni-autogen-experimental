...
# E/G weekly blocks (Mon-Sat) if block_days=6
    ...
    if "rules" in cfg and "I" in cfg["rules"] and "De Gregorio" in doctors:
        ...
        if max_i > 0:
            vars_ = []
            for s in slots:
                if s.columns == ["I"] and getattr(s, "rule_tag", "") == "I" and (s.slot_id, "De Gregorio") in x:
...
# ------------------------------------------------------------
    ...
    if MonT_need_rec and MonT_used_rec != MonT_need_rec:
        conflicts.append(f"T=Recupero su lunedì: attesi {MonT_need_rec}, ottenuti {MonT_used_rec}")
