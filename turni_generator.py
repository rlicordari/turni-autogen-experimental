    ...
    for day in days:
        ...
        if festivo:
            ...
            if "D_F" in rules and dayspec_contains(day.dow, rules["D_F"].get("days")):
                ...
                # Ultimate fallback (avoid infeasible/blank): any doctor (including Recupero if really needed)
                any_pool = apply_unavailability(sorted(doctors_set), day, "Mattina", unav)

                # Decide domains and whether D must equal F
                force_same = False
                if len(pair_avail) >= 2:
                    # Normal case: D/F are Grimaldi + Calabrò (not equal)
                    allowed_d = pair_avail
                    allowed_f = pair_avail
                elif len(pair_avail) == 1:
                    # Only one of the pair is schedulable: D uses the available one.
                    allowed_d = pair_avail
                    if h_avail:
                        # Prefer (and typically use) H pool for F; keep a tiny safety-net to avoid infeasible
                        allowed_f = sorted({*h_avail, *pair_avail, *any_pool})
                    else:
                        # H pool empty -> F must be the same as D
                        allowed_f = pair_avail
                        force_same = True
                else:
                    # Both pair docs unavailable -> pick from H pool; if H pool empty, last resort any doctor.
                    allowed_d = h_avail[:] if h_avail else (any_pool[:] if any_pool else sorted(doctors_set))
                    allowed_f = allowed_d[:]
                    force_same = True

                # Safety: never allow empty domain for required slots
                if not allowed_d:
                    allowed_d = sorted(doctors_set)
                if not allowed_f:
                    allowed_f = sorted(doctors_set)

                slots.append(Slot(day, f"{day.date}-D", ["D"], allowed_d, required=True, shift="Mattina", rule_tag="D_F.D", force_same_doctor=force_same))
                slots.append(Slot(day, f"{day.date}-F", ["F"], allowed_f, required=True, shift="Mattina", rule_tag="D_F.F", force_same_doctor=force_same))
# EG paired (Mon-Sat)
    ...
    for day in days:
        ...
        if enable_df_share and slotD is not None and slotF is not None:
            ...
            if df_y_by_doc:
                ...
                extra_obj.append(sum(df_y_by_doc.values()) * df_share_penalty)
...
# ---- H/I: divieto di stesso medico in giorni consecutivi (H e I indipendenti)
    ...
    if "rules" in cfg and "K" in cfg["rules"] and cfg["rules"]["K"].get("no_consecutive_days_same_doctor", False):
        for i in range(len(days)-1):
            ...
            for doc in doctors:
                ...
                if v1 is not None and v2 is not None:
                    model.Add(v1 + v2 <= 1)
    # D/F 3+3 weekly pattern + robust fallback (H.pool_mon_fri) + forced D=F when the pair is unavailable
    ...
    if "rules" in cfg and "D_F" in cfg["rules"] and isinstance(cfg["rules"]["D_F"], dict):
        ...
        if rDF.get("pattern_3_3", False):
            ...
            # When the pair isn't feasible, we want to pull from H.pool_mon_fri (and ONLY if that is also infeasible, go elsewhere).
            hpool = set(norm_name(d) for d in ((cfg.get("rules", {}).get("H", {}) or {}).get("pool_mon_fri") or []))
            outside_hpool_pen = int(rDF.get("outside_hpool_penalty", 2000) or 2000)
            ...
            for day in days:
                ...
                force_same = bool(getattr(sD, "force_same_doctor", False)) and bool(getattr(sF, "force_same_doctor", False))

                # If forced, D and F must be the SAME doctor (used when both Grimaldi/Calabrò are unavailable,
                # or when H.pool_mon_fri is empty and we must avoid blank/infeasible).
                if force_same:
                    inter = set(sD.allowed).intersection(set(sF.allowed))
                    for doc in inter:
                        vD = x.get((sD.slot_id, doc))
                        vF = x.get((sF.slot_id, doc))
                        if vD is not None and vF is not None:
                            model.Add(vD == vF)
                    # Prefer staying within H.pool_mon_fri if any such option exists
                    if any(doc in hpool for doc in inter):
                        for doc in inter:
                            if doc not in hpool:
                                v = x.get((sD.slot_id, doc))
                                if v is not None:
                                    extra_obj.append(outside_hpool_pen * v)
                    continue

                # Detect whether BOTH pair doctors are available in the domain (=> must use the pair)
                pair_possible = (doc1 in sD.allowed and doc2 in sD.allowed and doc1 in sF.allowed and doc2 in sF.allowed)

                if pair_possible:
                    # Enforce: {D, F} = {doc1, doc2} (a permutation)  -> never use others, never D=F here.
                    ...
                    # SOFT: prefer the weekly pattern when possible (only meaningful when the pair is feasible)
                    ...
                    # Pair isn't feasible: prefer choosing from H.pool_mon_fri (if any candidate exists in the domain).
                    # This avoids "random free doctor" behavior and keeps the fallback aligned with your workflow.
                    has_h = any(doc in hpool for doc in sF.allowed)
                    if has_h:
                        for doc in sF.allowed:
                            v = x.get((sF.slot_id, doc))
                            if v is not None and doc not in hpool:
                                extra_obj.append(outside_hpool_pen * v)

                    has_h_d = any(doc in hpool for doc in sD.allowed)
                    if has_h_d:
                        for doc in sD.allowed:
                            v = x.get((sD.slot_id, doc))
                            if v is not None and doc not in hpool:
