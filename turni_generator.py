...
def solve_with_ortools(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    for day in days:
        ...
        for d in doctors:
            vars_ = []
            for s in day_slots:
                if any(str(c).strip().upper() in daily_exempt_cols for c in s.columns):
                    continue
                if (s.slot_id, d) in x:
    ...
    # --- C (Reperibilità) spacing + balancing (target/max) ---
    # Note: max_per_doctor is treated as a SOFT cap by default (penalized if exceeded),
    # because with a small eligible pool it may be mathematically impossible to keep everyone <= max.
    if "rules" in cfg and "C_reperibilita" in cfg["rules"]:
        rC2 = cfg["rules"].get("C_reperibilita") or {}
        # Build C vars by (day, doc)
        c_var_by_day_doc = {}
        c_slots = [s for s in slots if s.columns == ["C"]]
        for s in c_slots:
            for doc in doctors:
                v = x.get((s.slot_id, doc))
                if v is not None:
                    c_var_by_day_doc[(s.day.date, doc)] = v
        eligible_c_docs = {doc for (_d, doc) in c_var_by_day_doc.keys()}
        eligible_c_docs.discard("Recupero")
        total_c = len(c_slots)

        # Hard spacing: at least spacing_min_days between two C for the same doctor
        spacing_min = int(rC2.get("spacing_min_days", 2) or 2)
        if spacing_min > 1:
            pos = {d.date: i for i, d in enumerate(days)}
            for doc in sorted(eligible_c_docs):
                for i, drow in enumerate(days):
                    v1 = c_var_by_day_doc.get((drow.date, doc))
                    if v1 is None:
                        continue
                    for k in range(1, spacing_min):
                        j = i + k
                        if j >= len(days):
                            break
        ...
        # Soft preferred spacing: discourage C closer than spacing_preferred_days
        spacing_pref = int(rC2.get("spacing_preferred_days", 0) or 0)
        if spacing_pref and spacing_pref > spacing_min:
            pen_close = int(rC2.get("spacing_preferred_penalty", 5) or 5)
            penalties = []
            for doc in sorted(eligible_c_docs):
                for i, drow in enumerate(days):
                    v1 = c_var_by_day_doc.get((drow.date, doc))
                    if v1 is None:
                        continue
                    for k in range(spacing_min, spacing_pref):
                        j = i + k
                        if j >= len(days):
                            break
                        v2 = c_var_by_day_doc.get((days[j].date, doc))
                        if v2 is None:
                            continue
                        b = model.NewBoolVar(f"Cclose_{hash(doc)%10**6}_{i}_{k}")
                        model.Add(v1 + v2 == 2).OnlyEnforceIf(b)
                        model.Add(v1 + v2 <= 1).OnlyEnforceIf(b.Not())
                        penalties.append(b)
            if penalties:
                extra_obj.append(pen_close * sum(penalties))

        # Balance: prefer target_per_doctor, penalize deviation
        target = rC2.get("target_per_doctor")
        max_pd = rC2.get("max_per_doctor")
        w_abs = int(rC2.get("balance_penalty", 12) or 12)
        w_short = int(rC2.get("under_target_penalty", 18) or 18)
        w_overmax = int(rC2.get("over_max_penalty", 40) or 40)
        hard_max = bool(rC2.get("max_per_doctor_hard", False))
        ...
        for doc in sorted(eligible_c_docs):
            ...
            if target is not None:
                t = int(target)
                # shortfall and excess relative to target (asymmetric weights)
                short = model.NewIntVar(0, total_c, f"Cshort_{hash(doc)%10**6}")
                over = model.NewIntVar(0, total_c, f"Cover_{hash(doc)%10**6}")
                model.Add(short >= t - cnt)
                model.Add(short >= 0)
                model.Add(over >= cnt - t)
                model.Add(over >= 0)
                # Encourage reaching the target (shortfall penalized a bit more)
                extra_obj.append(w_short * short)
                extra_obj.append(w_abs * over)

            if max_pd is not None:
                m = int(max_pd)
                if hard_max:
                    model.Add(cnt <= m)
                # Soft overflow penalty (allows exceeding if otherwise infeasible)
                overm = model.NewIntVar(0, total_c, f"Coverm_{hash(doc)%10**6}")
                model.Add(overm >= cnt - m)
                model.Add(overm >= 0)
                extra_obj.append(w_overmax * overm)
...
def solve_greedy(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    reperC_by_doc: Dict[str, List[dt.date]] = defaultdict(list)  # C reperibilità dates per doctor
    ...
    # --- C (Reperibilità) greedy controls (spacing + balancing) ---
    rC = rules.get("C_reperibilita") if isinstance(rules.get("C_reperibilita"), dict) else {}
    C_target = int(rC.get("target_per_doctor", 0) or 0)
    C_max = rC.get("max_per_doctor")
    C_max = int(C_max) if C_max is not None else None
    C_spacing_min = int(rC.get("spacing_min_days", 2) or 2)
    C_spacing_pref = int(rC.get("spacing_preferred_days", 0) or 0)
    C_constraints = set(rC.get("constraints") or [])
    # If enabled, block assigning C to someone who has a night on day d (same day)
    # and optionally also on d+1 and d+2.
    C_block_night_days = None
    if "not_night_next_2_days" in C_constraints:
        C_block_night_days = 2
    elif "not_night_same_day" in C_constraints:
        C_block_night_days = 0

    # Determine whether a HARD max is feasible (only then we enforce it in greedy)
    C_slots_total = sum(1 for s in slots if s.columns == ["C"])
    C_pool = set()
    for s in slots:
        if s.columns == ["C"]:
            C_pool.update([d for d in s.allowed if d != "Recupero"])
    C_max_hard = bool(rC.get("max_per_doctor_hard", False))
    C_max_hard_feasible = bool(C_max_hard and C_max is not None and C_pool and (len(C_pool) * C_max >= C_slots_total))

    def can_assign(s: Slot, doc: str) -> bool:
        # per-day uniqueness (ignore placeholder 'Recupero')
        # Some columns (e.g. C reperibilità) can be configured to be EXEMPT from daily uniqueness
        # by listing them in global_constraints.daily_uniqueness_exempt_columns.
        slot_exempt = any(str(c).strip().upper() in daily_exempt_cols for c in s.columns)
        ...
        # C reperibilità: spacing + (optional) night constraints
        if s.columns == ["C"]:
            ...
            if C_spacing_min and C_spacing_min > 1:
                for prev in reperC_by_doc.get(doc, []):
                    if abs((s.day.date - prev).days) < C_spacing_min:
                        return False
            # optional: avoid nights on same day and the next 2 days (depending on config)
            if C_block_night_days is not None:
                for off in range(0, int(C_block_night_days) + 1):
                    dcheck = s.day.date + dt.timedelta(days=off)
                    if dcheck in nights_by_doc.get(doc, []):
                        return False
            # optional hard max (only if mathematically feasible)
            if C_max_hard_feasible and C_max is not None:
                if len(reperC_by_doc.get(doc, [])) >= C_max:
                    return False
    ...
    def score_candidate(s: Slot, doc: str) -> Tuple:
        ...
        if s.columns == ["C"]:
            c_cnt = len(reperC_by_doc.get(doc, []))
            # prioritize bringing everyone up to target first
            under_flag = 0 if (C_target and c_cnt < C_target) else 1
            # soft cap overflow (penalize exceeding max, but don't block unless hard+feasible)
            over_flag = 0
            if C_max is not None and (c_cnt + 1) > C_max:
                over_flag = (c_cnt + 1) - C_max
            # prefer spreading over the month
            dist_pen = 0
            if C_spacing_pref and reperC_by_doc.get(doc):
                last = max(reperC_by_doc[doc])
                delta = abs((s.day.date - last).days)
                if delta < C_spacing_pref:
                    dist_pen = C_spacing_pref - delta
            return (under_flag, over_flag, dist_pen, c_cnt, load_total[doc], doc.lower())
    ...
    for s in slots_sorted:
        ...
        if chosen is None:
            ...
            continue
        assignment[s.slot_id] = chosen
        if not any(str(c).strip().upper() in daily_exempt_cols for c in s.columns):
