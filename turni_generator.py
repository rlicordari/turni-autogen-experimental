            ...
            for day in days:
                ...
                for doc in doctors:
                    ...
                    for off in [0,1,2]:
                        ...
                        if nvar is not None:
                            model.Add(nvar == 0).OnlyEnforceIf(cvar)
    # Reperibilita balance/spacing (soft), to avoid extreme imbalances (e.g., 5 vs 1)
    # Desired behavior (default): ~2 per doctor, allow 3 only if needed, never consecutive;
    # also prefer spacing of assignments across the month.
    if any(s.columns == ["C"] for s in slots):
        rC = ((cfg.get("rules") or {}).get("C_reperibilita") or {})
        try:
            target_c = int(rC.get("target_per_doctor", 2) or 2)
        except Exception:
            target_c = 2
        try:
            max_c = int(rC.get("max_per_doctor", 3) or 3)
        except Exception:
            max_c = 3
        try:
            min_gap_c = int(rC.get("spacing_min_days", 2) or 2)
        except Exception:
            min_gap_c = 2
        try:
            pref_gap_c = int(rC.get("spacing_preferred_days", 7) or 7)
        except Exception:
            pref_gap_c = 7
        pen_abs = int(rC.get("balance_abs_penalty", 30) or 30)
        pen_excess = int(rC.get("balance_excess_penalty", 250) or 250)
        pen_short = int(rC.get("balance_shortfall_penalty", 400) or 400)
        pen_close = int(rC.get("spacing_penalty", 8) or 8)

        # Map C vars by (day, doc)
        c_var_by_day_doc = {}
        for day in days:
            ...
            if not c_slot:
                continue
            for doc in doctors:
                v = x.get((c_slot.slot_id, doc))
                if v is not None:
                    c_var_by_day_doc[(day.date, doc)] = v

        c_pool = sorted({doc for (_d, doc) in c_var_by_day_doc.keys() if doc != "Recupero"})
        total_c = sum(1 for s in slots if s.columns == ["C"])
        if c_pool and total_c > 0:
            # Ensure max_c is at least what is mathematically necessary
            needed_max = (total_c + len(c_pool) - 1) // len(c_pool)
            max_c_eff = max(max_c, needed_max)
        else:
            max_c_eff = max_c

        # Hard: no consecutive (or too-close) reperibilita assignments
        for doc in c_pool:
            for i, drow in enumerate(days):
                v1 = c_var_by_day_doc.get((drow.date, doc))
                if v1 is None:
                    continue
                for k in range(1, max(2, min_gap_c)):
                    j = i + k
                    if j >= len(days):
                        break
        ...
        # Soft: keep counts close to target, discourage <target and >max
        for doc in c_pool:
            vars_ = []
            for drow in days:
                v = c_var_by_day_doc.get((drow.date, doc))
                if v is not None:
                    vars_.append(v)
            if not vars_:
                continue
            ...
            diff = model.NewIntVar(0, total_c, f"Cdiff_{hash(doc)%10**6}")
            model.AddAbsEquality(diff, cnt - target_c)
            extra_obj.append(pen_abs * diff)

            # excess over max_c_eff
            tmp_ex = model.NewIntVar(-total_c, total_c, f"Ctmp_ex_{hash(doc)%10**6}")
            model.Add(tmp_ex == cnt - max_c_eff)
            excess = model.NewIntVar(0, total_c, f"Cexcess_{hash(doc)%10**6}")
            model.AddMaxEquality(excess, [tmp_ex, model.NewConstant(0)])
            extra_obj.append(pen_excess * excess)

            # shortfall below target
            tmp_sh = model.NewIntVar(-total_c, total_c, f"Ctmp_sh_{hash(doc)%10**6}")
            model.Add(tmp_sh == target_c - cnt)
            short = model.NewIntVar(0, total_c, f"Cshort_{hash(doc)%10**6}")
            model.AddMaxEquality(short, [tmp_sh, model.NewConstant(0)])
            extra_obj.append(pen_short * short)

            # Soft spacing: penalize close pairs within pref_gap_c days
            if pref_gap_c and pref_gap_c > 1 and pen_close > 0:
                for i, drow in enumerate(days):
                    v1 = c_var_by_day_doc.get((drow.date, doc))
                    if v1 is None:
                        continue
                    for k in range(1, pref_gap_c):
                        j = i + k
                        if j >= len(days):
                            break
                        v2 = c_var_by_day_doc.get((days[j].date, doc))
...
def solve_greedy(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    # Reperibilita (C) balancing knobs (greedy)
    rC = rules.get("C_reperibilita", {}) if isinstance(rules.get("C_reperibilita"), dict) else {}
    try:
        c_target = int(rC.get("target_per_doctor", 2) or 2)
    except Exception:
        c_target = 2
    try:
        c_max = int(rC.get("max_per_doctor", 3) or 3)
    except Exception:
        c_max = 3
    try:
        c_min_gap = int(rC.get("spacing_min_days", 2) or 2)
    except Exception:
        c_min_gap = 2
    try:
        c_pref_gap = int(rC.get("spacing_preferred_days", 7) or 7)
    except Exception:
        c_pref_gap = 7
    # If math requires > c_max, allow it but keep preference via scoring.
    c_slots_total = sum(1 for s in slots if s.columns == ["C"])
    c_pool = sorted({d for s in slots if s.columns == ["C"] for d in s.allowed if d != "Recupero"})
    if c_pool:
        needed_max = (c_slots_total + len(c_pool) - 1) // len(c_pool)
        c_max_eff = max(c_max, needed_max)
    else:
        c_max_eff = c_max
    ...
    def can_assign(s: Slot, doc: str) -> bool:
        ...
        # Reperibilita (C) spacing: never consecutive (or too-close) for the same doctor
        if s.columns == ["C"]:
            # Check last assigned C for this doctor
            if c_dates_by_doc[doc]:
                last = c_dates_by_doc[doc][-1]
                if abs((s.day.date - last).days) < max(1, c_min_gap):
                    return False
    ...
    def score_candidate(s: Slot, doc: str) -> Tuple:
        ...
        if s.columns == ["C"]:
            c_cnt = len(c_dates_by_doc[doc])
            days_since = 999
            if c_dates_by_doc[doc]:
                days_since = abs((s.day.date - c_dates_by_doc[doc][-1]).days)
            # Tiers: prefer doctors < target; then < max; avoid over-max if possible.
            tier = 0 if c_cnt < c_target else (1 if c_cnt < c_max_eff else 2)
            close_flag = 1 if days_since < c_pref_gap else 0
            # Lower is better; prefer larger spacing (hence -days_since)
            return (tier, close_flag, c_cnt, -days_since, load_total[doc], doc.lower())
