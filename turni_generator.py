...
# -------------------------
def assign_reperibilita_C(cfg: dict, days: List[DayRow], slots: List[Slot], assignment: Dict[str, Optional[str]]) -> Tuple[Dict[str, Optional[str]], Dict]:
    """Assign/Reassign Reperibilità (column C) as a final layer.

    Rules (hard whenever feasible):
    ...
    - Per doctor per month: min_per_doctor (typically 2) and max_per_doctor (typically 3)
    - Minimum spacing between two C for the same doctor: spacing_min_days (typically 3)

    If strict constraints are infeasible (rare: too few eligible doctors, extreme unavailability),
    the algorithm relaxes in a controlled way and reports diagnostics in stats.
    """
    rules = cfg.get("rules", {}) or {}
    rC = rules.get("C_reperibilita", {}) if isinstance(rules.get("C_reperibilita", {}), dict) else {}
    if not rC:
        return assignment, {"C_reperibilita_diag": {"status": "not_configured"}}

    constraints = set(rC.get("constraints") or [])
    excluded = {norm_name(x) for x in (rC.get("excluded") or [])}
    spacing_min = int(rC.get("spacing_min_days", 3) or 3)
    ...
    night_col = "J"  # fixed per your convention
    ...
    if not c_dates:
        return assignment, {"C_reperibilita_diag": {"status": "no_C_slots"}}

    allowed_norm_by_date: Dict[dt.date, List[str]] = {}
    pool_set: Set[str] = set()
    for d in c_dates:
        cand = []
        for doc in (cslot_by_date[d].allowed or []):
            dn = norm_name(doc)
            if dn and dn not in excluded and dn != "Recupero":
                cand.append(dn)
        seen=set()
        cand2=[]
        for x in cand:
    ...
    if not pool:
        for d in c_dates:
            assignment[cslot_by_date[d].slot_id] = None
        return assignment, {"C_reperibilita_diag": {"status": "empty_pool"}}
    ...
    def ok_candidate(date_: dt.date, doc: str, relax_holiday_work: bool) -> bool:
        ...
        if "not_night_prev_day" in constraints:
            ...
            if assignment.get(f"{prev}-{night_col}") == doc:
                return False
        if (not relax_holiday_work) and "not_working_same_day_on_sundays_and_holidays" in constraints:
    ...
    def build_candidates(relax_holiday_work: bool) -> Dict[dt.date, List[str]]:
        return {d: [doc for doc in pool if ok_candidate(d, doc, relax_holiday_work)] for d in c_dates}

    total_days = len(c_dates)
    n_docs = len(pool)

    strict_count_feasible = True
    if max_per > 0 and total_days > n_docs * max_per:
        strict_count_feasible = False
    if min_per > 0 and total_days < n_docs * min_per:
        strict_count_feasible = False

    desired = {doc: 0 for doc in pool}
    if strict_count_feasible and target and max_per:
        for doc in pool:
            desired[doc] = min(target, max_per)
        cur = sum(desired.values())
        if cur > total_days:
            for doc in reversed(pool):
                while desired[doc] > 0 and cur > total_days:
                    desired[doc] -= 1
                    cur -= 1
        elif cur < total_days:
            rem = total_days - cur
        ...
        base = total_days // n_docs
        rem = total_days - base * n_docs
        for i, doc in enumerate(pool):
            desired[doc] = base + (1 if i < rem else 0)
        if max_per:
            for doc in pool:
                desired[doc] = min(desired[doc], max_per)

    def solve_once(relax_holiday_work: bool, spacing: int, max_cap: int) -> Tuple[bool, Dict[dt.date, str]]:
        cands_by_date = build_candidates(relax_holiday_work)
        order = sorted(c_dates, key=lambda d: (len(cands_by_date[d]), d))
        if any(len(cands_by_date[d]) == 0 for d in order):
            return False, {}
        ...
        local_desired = dict(desired)
        if max_cap:
            for doc in pool:
                local_desired[doc] = min(local_desired.get(doc, 0), max_cap)

        def spacing_ok(doc: str, d: dt.date) -> bool:
            if spacing and spacing > 1:
                for prev in dates_by_doc[doc]:
                    if abs((d - prev).days) < spacing:
                        return False
            return True

        def pick_order_for_day(d: dt.date) -> List[str]:
            cands = cands_by_date[d]
            def key(doc):
                return (local_desired.get(doc, 0) - cnt[doc], -cnt[doc], doc.lower())
        ...
        def dfs(i: int) -> bool:
            if i == len(order):
                return True
            d = order[i]
            for doc in pick_order_for_day(d):
                if max_cap and cnt[doc] >= max_cap:
                    continue
                if strict_count_feasible and local_desired.get(doc, 0) and cnt[doc] >= local_desired[doc]:
                    continue
        ...
        ok = dfs(0)
        return ok, assigned if ok else {}

    diag = {
        "status": "unknown",
        "strict_count_feasible": strict_count_feasible,
        "pool_size": n_docs,
        "total_days": total_days,
        "constraints": sorted(constraints),
        "requested": {"min_per_doctor": min_per, "max_per_doctor": max_per, "spacing_min_days": spacing_min},
    }

    attempts = [
        (False, spacing_min, max_per or 0),
        (True, spacing_min, max_per or 0),
    ]
    if spacing_min > 2:
        attempts.append((True, 2, max_per or 0))
    if spacing_min > 1:
        attempts.append((True, 1, max_per or 0))
    if max_per:
        attempts.append((True, max(1, spacing_min - 1), max_per + 1))

    solved = False
    chosen = None
    amap: Dict[dt.date, str] = {}
    for relax_holiday, spacing, maxcap in attempts:
        ok, candidate_map = solve_once(relax_holiday, spacing, maxcap)
        if ok:
            solved = True
            chosen = {"relax_holiday_work": relax_holiday, "spacing": spacing, "max_cap": maxcap}
            amap = candidate_map
            diag["status"] = "OK_STRICT" if chosen == {"relax_holiday_work": False, "spacing": spacing_min, "max_cap": (max_per or 0)} else "OK_RELAXED"
            diag["chosen"] = chosen
            break

    for d in c_dates:
        assignment[cslot_by_date[d].slot_id] = amap.get(d)

    if not solved:
        diag["status"] = "FAILED"
        diag["reason"] = "Could not assign C under constraints; C left blank for some/all days."

    # Post-check counts / spacing for diagnostics
    cnt_final = Counter([v for v in amap.values() if v])
    diag["counts"] = dict(cnt_final)
