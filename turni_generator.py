...
def solve_with_ortools(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    # Allow some columns (e.g., C=Reperibilità) to bypass the "one doctor per day" uniqueness rule.
    exempt_cols = set(gc.get("daily_uniqueness_exempt_columns") or [])
    for day in days:
        day_slots = slots_by_day.get(day.date, [])
        ...
        for d in doctors:
            vars_ = []
            for s in day_slots:
                # Skip exempt columns (e.g. C) from daily uniqueness accounting
                if any(col in exempt_cols for col in (s.columns or [])):
                    continue
                if (s.slot_id, d) in x:
    ...
    # Reperibilità (C): spacing + fair distribution (soft min/target/max with penalties)
    if "rules" in cfg and "C_reperibilita" in cfg["rules"]:
        rC = cfg["rules"]["C_reperibilita"] or {}
        excluded = {norm_name(x) for x in (rC.get("excluded") or [])}
        eligible_docs = [d for d in doctors if d not in excluded and d != "Recupero"]

        # Parameters (defaults tuned for 28-day months): target=2, allow 3 only if needed, spacing>=3 days
        target = int(rC.get("target_per_doctor", 2))
        min_target = int(rC.get("min_per_doctor", target))  # optional; treated as target floor via penalty
        max_per = int(rC.get("max_per_doctor", 3))
        spacing_min = int(rC.get("spacing_min_days", 3))
        spacing_pref = int(rC.get("spacing_preferred_days", spacing_min))

        under_pen = int(rC.get("under_target_penalty", 80))
        over_pen = int(rC.get("over_max_penalty", 200))
        pref_pen = int(rC.get("preferred_spacing_penalty", 10))
        balance_pen = int(rC.get("balance_penalty", 10))

        # Locate daily C slot
        cslot_by_date: Dict[dt.date, Optional[Slot]] = {}
        for day in days:
            cslot_by_date[day.date] = next((s for s in slots_by_day[day.date] if s.columns == ["C"]), None)

        pos = {d.date: i for i, d in enumerate(days)}
        # Map (doc, day_index) -> BoolVar
        cvar: Dict[Tuple[str, int], cp_model.IntVar] = {}
        for day in days:
            cs = cslot_by_date.get(day.date)
            if not cs:
                continue
            i = pos[day.date]
            for doc in eligible_docs:
                v = x.get((cs.slot_id, doc))
                if v is not None:
                    cvar[(doc, i)] = v

        total_c_days = sum(1 for day in days if cslot_by_date.get(day.date) is not None)

        # Spacing hard: in any window of size spacing_min, a doctor can appear at most once
        if spacing_min and spacing_min > 1:
            for doc in eligible_docs:
                for i in range(0, len(days) - spacing_min + 1):
                    window = [cvar[(doc, j)] for j in range(i, i + spacing_min) if (doc, j) in cvar]
                    if len(window) >= 2:
                        model.Add(sum(window) <= 1)

        # Soft distribution: push each doctor toward ~target (2), penalize <target and >max_per
        for doc in eligible_docs:
            vars_doc = [cvar[(doc, i)] for i in range(len(days)) if (doc, i) in cvar]
            if not vars_doc:
                continue
            cnt = sum(vars_doc)

            under = model.NewIntVar(0, total_c_days, f"c_under_{hash(doc)%10**6}")
            model.Add(under >= min_target - cnt)
            model.Add(under >= 0)

            over = model.NewIntVar(0, total_c_days, f"c_over_{hash(doc)%10**6}")
            model.Add(over >= cnt - max_per)
            model.Add(over >= 0)

            dev = model.NewIntVar(0, total_c_days, f"c_dev_{hash(doc)%10**6}")
            model.AddAbsEquality(dev, cnt - target)

            extra_obj.append(under * under_pen)
            extra_obj.append(over * over_pen)
            extra_obj.append(dev * balance_pen)

        # Soft preferred spacing (optional): penalize repeats closer than spacing_pref (but >= spacing_min)
        if spacing_pref and spacing_pref > spacing_min:
            for doc in eligible_docs:
                for i in range(0, len(days) - spacing_pref):
                    for j in range(i + spacing_min, min(i + spacing_pref, len(days))):
                        if (doc, i) in cvar and (doc, j) in cvar:
                            both = model.NewBoolVar(f"c_close_{hash(doc)%10**6}_{i}_{j}")
                            model.Add(both <= cvar[(doc, i)])
                            model.Add(both <= cvar[(doc, j)])
                            model.Add(both >= cvar[(doc, i)] + cvar[(doc, j)] - 1)
                            extra_obj.append(both * pref_pen)
...
def solve_greedy(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    # Daily uniqueness exemptions (e.g. C=Reperibilità can coexist with other tasks)
    exempt_cols = set((cfg.get("global_constraints") or {}).get("daily_uniqueness_exempt_columns") or [])

    # C: Reperibilità balancing/spacing parameters (enforced also in greedy fallback)
    rC = rules.get("C_reperibilita") if isinstance(rules.get("C_reperibilita"), dict) else {}
    C_target = int(rC.get("target_per_doctor", 2)) if rC else 2
    C_min_target = int(rC.get("min_per_doctor", C_target)) if rC else C_target
    C_max = int(rC.get("max_per_doctor", 3)) if rC else 3
    C_spacing = int(rC.get("spacing_min_days", 3)) if rC else 3
    rep_days_by_doc = defaultdict(list)

    def can_assign(s: Slot, doc: str) -> bool:
        # per-day uniqueness (ignore placeholder 'Recupero') unless column is exempt (e.g., C)
        if doc != "Recupero" and doc in used_per_day[s.day.date] and not any(col in exempt_cols for col in (s.columns or [])):
            return False
        # L cap for Recupero
        ...
        # C (Reperibilità): enforce minimum spacing and soft max via selection (hard cap here to guarantee fairness)
        if s.columns == ["C"] and rC:
            # spacing hard
            for prev in rep_days_by_doc[doc]:
                if abs((s.day.date - prev).days) < C_spacing:
                    return False
            # hard cap to keep within max (should be feasible with full pool + C exemption)
            if len(rep_days_by_doc[doc]) >= C_max:
                return False
    ...
    def score_candidate(s: Slot, doc: str) -> Tuple:
        ...
        if s.columns == ["C"] and rC:
            # Prefer doctors with fewer reperibilità; push toward target (2) and keep total load balanced
            c_cnt = len(rep_days_by_doc[doc])
            return (c_cnt, abs(c_cnt - C_target), load_total[doc], doc.lower())
    ...
    for s in slots_sorted:
        ...
        if s.columns == ["C"] and rC:
            rep_days_by_doc[chosen].append(s.day.date)
