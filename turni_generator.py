...
def slots_for_month(cfg: dict, days: List[DayRow], unav: Dict[str, Dict[dt.date, Set[str]]]) -> List[Slot]:
    ...
    # Columns that are allowed to co-exist with another assignment in the same day (do NOT count toward uniqueness)
    exempt_cols = {str(c).strip().upper() for c in (gc.get("daily_uniqueness_exempt_columns") or []) if str(c).strip()}
...
def solve_with_ortools(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    for day in days:
        ...
        for d in doctors:
            vars_ = []
            for s in day_slots:
                # Allow some columns (e.g., C reperibilità) to NOT count toward daily uniqueness
                if exempt_cols and all(str(cc).strip().upper() in exempt_cols for cc in (s.columns or [])):
                    continue
                if (s.slot_id, d) in x:
    ...
    # C spacing / cap for Reperibilità
        if s.columns == ["C"] and doc != "Recupero":
            if c_spacing_min and c_dates_by_doc[doc]:
                last = max(c_dates_by_doc[doc])
                if abs((s.day.date - last).days) < c_spacing_min:
                    return False
            if c_max and c_max_hard and c_count[doc] >= c_max:
                return False
# Night off next day
    gc = cfg.get("global_constraints", {}) or {}
            ...
            for day in days:
                ...
                for doc in doctors:
                    ...
                    for off in [0,1,2]:
                        ...
                        if nvar is not None:
                            model.Add(nvar == 0).OnlyEnforceIf(cvar)
    # ---- C (Reperibilità) balancing + spacing (per month)
    if "rules" in cfg and "C_reperibilita" in cfg["rules"]:
        rC = cfg["rules"]["C_reperibilita"]
        try:
            c_min = int(rC.get("min_per_doctor", 0) or 0)
        except Exception:
            c_min = 0
        try:
            c_target = int(rC.get("target_per_doctor", c_min) or c_min)
        except Exception:
            c_target = c_min
        try:
        ...
        min_hard = bool(rC.get("min_per_doctor_hard", False))
        max_hard = bool(rC.get("max_per_doctor_hard", False))
        bal_pen = int(rC.get("balance_penalty", 80))
        under_pen = int(rC.get("under_min_penalty", 500))
        over_pen = int(rC.get("over_max_penalty", 2000))
        pref_pen = int(rC.get("preferred_spacing_penalty", 10))

        # Map day->C slot (if present)
        c_slot_by_day: Dict[dt.date, Slot] = {}
        for day in days:
            cs = next((s for s in slots_by_day.get(day.date, []) if s.columns == ["C"]), None)
            if cs is not None:
                c_slot_by_day[day.date] = cs

        # Eligible docs for C are those that appear in any C decision var
        c_docs: Set[str] = set()
        for dte, cs in c_slot_by_day.items():
            for doc in doctors:
                if (cs.slot_id, doc) in x:
                    c_docs.add(doc)

        if c_docs and c_slot_by_day:
            # Group by (year, month) for per-month balancing
            by_month: Dict[Tuple[int, int], List[dt.date]] = defaultdict(list)
            for day in days:
                by_month[(day.date.year, day.date.month)].append(day.date)

            for (yy, mm), dlist in by_month.items():
                dlist = sorted(dlist)
                S = len(dlist)
                docs_m = sorted(c_docs)
                N = len(docs_m)
                if S <= 0 or N <= 0:
                    continue

                # Feasibility guardrails: if hard min/max impossible, fall back to soft penalties
                min_hard_eff = bool(min_hard and c_min and (c_min * N <= S))
                max_hard_eff = bool(max_hard and c_max and (c_max * N >= S))

                # Spacing HARD: if configured, prevent two C assignments too close (difference < c_spacing_min)
                if c_spacing_min and c_spacing_min > 1:
                    pos_map = {d: i for i, d in enumerate(dlist)}
                    for doc in docs_m:
                        for i, d1 in enumerate(dlist):
                            cs1 = c_slot_by_day.get(d1)
                            if cs1 is None or (cs1.slot_id, doc) not in x:
                                continue
                            v1 = x[(cs1.slot_id, doc)]
                            for k in range(1, c_spacing_min):
                                j = i + k
                                if j >= len(dlist):
                                    break
                                d2 = dlist[j]
                                cs2 = c_slot_by_day.get(d2)
                                if cs2 is None or (cs2.slot_id, doc) not in x:
                                    continue
                                v2 = x[(cs2.slot_id, doc)]
                                model.Add(v1 + v2 <= 1)

                # Soft spacing preference: penalize gaps that are < c_spacing_pref but >= c_spacing_min
                # (kept light; optional)
                if c_spacing_pref and c_spacing_pref > max(c_spacing_min, 1):
                    for doc in docs_m:
                        for i, d1 in enumerate(dlist):
                            cs1 = c_slot_by_day.get(d1)
                            if cs1 is None or (cs1.slot_id, doc) not in x:
                                continue
                            v1 = x[(cs1.slot_id, doc)]
                            for k in range(1, c_spacing_pref):
                                j = i + k
                                if j >= len(dlist):
                                    break
                                d2 = dlist[j]
                                ...
                                if cs2 is None or (cs2.slot_id, doc) not in x:
                                    continue
                                # If both exist, penalize them being both 1 (soft)
                                v2 = x[(cs2.slot_id, doc)]
                                p = model.NewBoolVar(f"c_close_{yy}{mm}_{i}_{k}_{hash(doc)%10**5}")
                                model.Add(p <= v1)
                                model.Add(p <= v2)
                                model.Add(p >= v1 + v2 - 1)
                                extra_obj.append(p * pref_pen)

                # Per-doctor counts and fairness
                for doc in docs_m:
                    vars_ = []
                    for dte in dlist:
                        cs = c_slot_by_day.get(dte)
                        if cs is None:
                            continue
                        if (cs.slot_id, doc) in x:
                            vars_.append(x[(cs.slot_id, doc)])
                    if not vars_:
                        continue
                    c_cnt = model.NewIntVar(0, S, f"c_cnt_{yy}{mm}_{hash(doc)%10**6}")
                    model.Add(c_cnt == sum(vars_))

                    # Min / Max constraints or soft penalties
                    if c_min and min_hard_eff:
                        model.Add(c_cnt >= c_min)
                    elif c_min:
                        under = model.NewIntVar(0, S, f"c_under_{yy}{mm}_{hash(doc)%10**6}")
                        model.Add(under >= c_min - c_cnt)
                        extra_obj.append(under * under_pen)

                    if c_max and max_hard_eff:
                        model.Add(c_cnt <= c_max)
                    elif c_max:
                        over = model.NewIntVar(0, S, f"c_over_{yy}{mm}_{hash(doc)%10**6}")
                        model.Add(over >= c_cnt - c_max)
                        extra_obj.append(over * over_pen)

                    # Balance to target
                    if c_target:
                        diff = model.NewIntVar(-S, S, f"c_diff_{yy}{mm}_{hash(doc)%10**6}")
                        absd = model.NewIntVar(0, S, f"c_abs_{yy}{mm}_{hash(doc)%10**6}")
                        model.Add(diff == c_cnt - c_target)
                        model.AddAbsEquality(absd, diff)
                        extra_obj.append(absd * bal_pen)
...
def solve_greedy(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    # C (Reperibilità): enforce spacing and per-doctor quotas (also in greedy fallback)
    rC = (cfg.get("rules") or {}).get("C_reperibilita") if isinstance((cfg.get("rules") or {}).get("C_reperibilita"), dict) else {}
    try:
        c_spacing_min = int(rC.get("spacing_min_days", 0) or 0)
    except Exception:
        c_spacing_min = 0
    ...
    c_dates_by_doc: Dict[str, List[dt.date]] = defaultdict(list)
    c_count = Counter()
    load_total = Counter()
    ...
    def can_assign(s: Slot, doc: str) -> bool:
        # per-day uniqueness (ignore placeholder 'Recupero') — allow exempt columns (e.g., C)
        if doc != "Recupero":
            is_exempt = bool(exempt_cols and all(str(cc).strip().upper() in exempt_cols for cc in (s.columns or [])))
            if (not is_exempt) and (doc in used_per_day[s.day.date]):
                return False
        # L cap for Recupero
...
# Night off next day (if doc did night previous day)
    ...
    for s in slots_sorted:
        ...
        if chosen is None:
            ...
            continue
        assignment[s.slot_id] = chosen
        # Update per-day usage only for non-exempt slots
        if chosen != "Recupero":
            is_exempt = bool(exempt_cols and all(str(cc).strip().upper() in exempt_cols for cc in (s.columns or [])))
            if not is_exempt:
                used_per_day[s.day.date].add(chosen)
        load_total[chosen] += 1
        # Track C assignments for spacing/quotas
        if s.columns == ["C"] and chosen != "Recupero":
            c_count[chosen] += 1
            c_dates_by_doc[chosen].append(s.day.date)
...
def solve_across_months(
    ...
    Expected schema for each key 'YYYY-MM':
      {
        "blocked_day1_doctors": ["Doctor A", ...],
        "recent_nights_by_doc": {"Doctor A": ["YYYY-MM-DD", ...], ...},
        "source_last_date": "YYYY-MM-DD"   # optional, for reporting
      }

    We translate carryover into additional unavailability constraints for the
    target month (day-1 blocked + night spacing).
    """
