...
def parse_date(x) -> dt.date:
    ...
    if isinstance(x, (int, float)):
        ...
        # 1 -> 1899-12-31 (Excel thinks 1900 is leap; using 1899-12-30 base works for modern dates).
        try:
            base = dt.date(1899, 12, 30)
            return base + dt.timedelta(days=int(x))
        except Exception:
...
def apply_unavailability(allowed: List[str], day: DayRow, shift: str, unav: Dict[str, Dict[dt.date, Set[str]]]) -> List[str]:
    ...
    for doc in allowed:
        ...
        out.append(doc)
    return out
def slots_for_month(cfg: dict, days: List[DayRow], unav: Dict[str, Dict[dt.date, Set[str]]], pre_nights_by_doc: Optional[Dict[str, List[dt.date]]] = None) -> List[Slot]:
    """
    ...
    # Carryover support for night-spacing across months (used to filter night candidates early in the month)
    pre_nights_by_doc = pre_nights_by_doc or {}
    gc0 = cfg.get("global_constraints", {}) or {}
    min_gap0 = int(gc0.get("night_spacing_days_min", 5) or 5)
    ...
    for day in days:
        ...
        if "J" in rules:
            ...
            if not (rJ.get("thursday_blank", False) and day.dow == "Thu"):
                ...
                # Carryover: if the doctor had a night in the previous (min_gap0-1) days (from previous month),
                # forbid early-month nights to respect spacing across month boundaries.
                if pre_nights_by_doc and min_gap0 and min_gap0 > 1:
                    def _ok(doc: str) -> bool:
                        for prev in pre_nights_by_doc.get(doc, []) or []:
                            try:
                                if (day.date - prev).days < min_gap0 and (day.date - prev).days >= 0:
                                    return False
                            except Exception:
                                continue
                        return True
                    allowed = [d for d in allowed if _ok(d)]
...
def solve_with_ortools(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    for day in days:
        ...
        for d in doctors:
            vars_ = []
            for s in day_slots:
                # Columns listed in global_constraints.daily_uniqueness_exempt_columns
                # do NOT count towards the 1-slot-per-day rule (e.g., C reperibilita can be combined with a day duty).
                if any(str(c).strip().upper() in daily_exempt_cols for c in (s.columns or [])):
                    continue
                if (s.slot_id, d) in x:
            ...
            for day in days:
                ...
                for doc in doctors:
                    ...
                    for off in [0,1,2]:
                        ...
                        if nvar is not None:
                            model.Add(nvar == 0).OnlyEnforceIf(cvar)
    # C (Reperibilita) spacing + balancing
    if "rules" in cfg and "C_reperibilita" in cfg["rules"] and isinstance(cfg["rules"].get("C_reperibilita"), dict):
        rC = cfg["rules"]["C_reperibilita"]
        C_spacing_min = int(rC.get("spacing_min_days", 0) or 0)
        C_spacing_pref = int(rC.get("spacing_preferred_days", 0) or 0)
        C_target = int(rC.get("target_per_doctor", 0) or 0)
        C_max = rC.get("max_per_doctor", None)
        try:
            C_max = int(C_max) if C_max is not None else None
        except Exception:
            C_max = None
        C_max_hard = bool(rC.get("max_per_doctor_hard", False))

        # Vars for C by (day,doc)
        c_var_by_day_doc = {}
        for day in days:
            c_slot = next((s for s in slots_by_day.get(day.date, []) if s.columns == ["C"]), None)
            if not c_slot:
                continue
            for doc in doctors:
                if (c_slot.slot_id, doc) in x:
                    c_var_by_day_doc[(day.date, doc)] = x[(c_slot.slot_id, doc)]

        # HARD spacing: no assignments closer than C_spacing_min days
        if C_spacing_min and C_spacing_min > 1:
            for doc in doctors:
                for i, drow in enumerate(days):
                    v1 = c_var_by_day_doc.get((drow.date, doc))
                    if v1 is None:
                        continue
                    for k in range(1, C_spacing_min):
                        j = i + k
                        if j >= len(days):
                            break
                        v2 = c_var_by_day_doc.get((days[j].date, doc))
                        if v2 is not None:
                            model.Add(v1 + v2 <= 1)

        # Pool actually eligible for C (from allowed lists)
        C_pool = set()
        for day in days:
            c_slot = next((s for s in slots_by_day.get(day.date, []) if s.columns == ["C"]), None)
            if c_slot:
                C_pool.update([d for d in c_slot.allowed if d in doctors and d != "Recupero"])
        total_C = sum(1 for day in days if next((s for s in slots_by_day.get(day.date, []) if s.columns == ["C"]), None) is not None)

        cnt_vars = []
        for doc in sorted(C_pool, key=lambda s: s.lower()):
            vars_ = [c_var_by_day_doc.get((d.date, doc)) for d in days]
            vars_ = [v for v in vars_ if v is not None]
            if not vars_:
                continue
            cnt = model.NewIntVar(0, total_C, f"Ccnt_{hash(doc)%10**6}")
            model.Add(cnt == sum(vars_))
            cnt_vars.append(cnt)

            # Encourage at least target (soft): penalize shortfall
            if C_target and C_target > 0:
                short = model.NewIntVar(0, max(C_target, total_C), f"Cshort_{hash(doc)%10**6}")
                model.Add(short >= C_target - cnt)
                model.Add(short >= 0)
                extra_obj.append(40 * short)

            # Soft cap max_per_doctor: penalize overflow (hard only if feasible)
            if C_max is not None and C_max > 0:
                over = model.NewIntVar(0, total_C, f"Cover_{hash(doc)%10**6}")
                model.Add(over >= cnt - C_max)
                model.Add(over >= 0)
                extra_obj.append(120 * over)

                # Hard max only if requested AND mathematically feasible
                if C_max_hard and len(C_pool) * C_max >= total_C:
                    model.Add(cnt <= C_max)

        # Fairness: minimize spread between max and min C-load
        if cnt_vars:
            cmax = model.NewIntVar(0, total_C, "C_max_load")
            cmin = model.NewIntVar(0, total_C, "C_min_load")
            model.AddMaxEquality(cmax, cnt_vars)
            model.AddMinEquality(cmin, cnt_vars)
            extra_obj.append(10 * (cmax - cmin))

        # SOFT preferred spacing (>= spacing_preferred_days): penalize assignments too close
        if C_spacing_pref and C_spacing_pref > max(1, C_spacing_min):
            for doc in C_pool:
                for i, drow in enumerate(days):
                    v1 = c_var_by_day_doc.get((drow.date, doc))
                    if v1 is None:
                        continue
                    for k in range(max(1, C_spacing_min), C_spacing_pref):
                        j = i + k
                        if j >= len(days):
                            break
                        v2 = c_var_by_day_doc.get((days[j].date, doc))
                        if v2 is None:
                            continue
                        b = model.NewBoolVar(f"Cgap_{hash(doc)%10**6}_{i}_{k}")
                        model.Add(v1 + v2 == 2).OnlyEnforceIf(b)
                        model.Add(v1 + v2 <= 1).OnlyEnforceIf(b.Not())
                        extra_obj.append(3 * b)
...
def solve_greedy(cfg: dict, days: List[DayRow], slots: List[Slot]) -> Tuple[Dict[str, Optional[str]], Dict]:
    ...
    shift_rank = {"Notte": 0, "Any": 1, "Pomeriggio": 2, "Mattina": 3}
    ...
    def can_assign(s: Slot, doc: str) -> bool:
        ...
        slot_exempt = any(str(c).strip().upper() in daily_exempt_cols for c in (s.columns or []))
        if (not slot_exempt) and doc != "Recupero" and doc in used_per_day[s.day.date]:
            return False
        # L cap for Recupero
        ...
        if "K" in s.columns and isinstance(rules.get("K"), dict) and rules["K"].get("no_consecutive_days_same_doctor", False):
            ...
            if assignment.get(f"{prev_day}-K") == doc:
                return False
        # C reperibilita: enforce spacing (no consecutive if spacing_min_days=2)
        if s.columns == ["C"] and C_spacing_min and C_spacing_min > 1:
            for prev in reperC_by_doc.get(doc, []):
                if abs((s.day.date - prev).days) < C_spacing_min:
                    return False
        # Optional soft cap for C (handled in scoring/choice; here only hard if explicitly feasible)
...
def write_solver_log(out_path: Path, stats: Dict) -> Optional[Path]:
    ...
    try:
        ...
        return None
def solve_across_months(cfg: dict, days: List[DayRow], unav_map: Dict[str, Dict[dt.date, Set[str]]], carryover_by_month: Optional[dict] = None) -> Tuple[List[Slot], Dict[str, Optional[str]], Dict]:
    """
    ...
    carryover_by_month = carryover_by_month or {}
    # Carryover nights from the immediately previous solved month (when the template contains multiple months)
    gc = cfg.get("global_constraints", {}) or {}
    min_gap = int(gc.get("night_spacing_days_min", 5) or 5)
    prev_nights_by_doc: Dict[str, List[dt.date]] = defaultdict(list)

    def _get_carryover_for(yy: int, mm: int) -> dict:
        key1 = (yy, mm)
        key2 = f"{yy:04d}-{mm:02d}"
        if isinstance(carryover_by_month, dict):
            if key1 in carryover_by_month:
                return carryover_by_month.get(key1) or {}
            if key2 in carryover_by_month:
                return carryover_by_month.get(key2) or {}
        return {}

    def _normalize_dates_map(m: dict) -> Dict[str, List[dt.date]]:
        out: Dict[str, List[dt.date]] = {}
        if not isinstance(m, dict):
            return out
        for doc, dates in m.items():
            docn = norm_name(doc)
            out.setdefault(docn, [])
            if isinstance(dates, (list, tuple, set)):
                for d in dates:
                    try:
                        out[docn].append(parse_date(d))
                    except Exception:
                        pass
            else:
                try:
                    out[docn].append(parse_date(dates))
                except Exception:
                    pass
        return out
    for (yy, mm) in month_keys:
        days_m = [d for d in days if (d.date.year, d.date.month) == (yy, mm)]
        # Apply carryover info (from GUI / previous-month output)
        cinfo = _get_carryover_for(yy, mm)
        first_day = min(d.date for d in days_m) if days_m else None

        # Doctors to block on the first day of the month (e.g., did NIGHT on last day of previous month)
        blocked = []
        for k in ["blocked_day1_doctors", "blocked_on_first_day", "blocked_on_first_day_doctors", "block_day1_doctors"]:
            if isinstance(cinfo.get(k), (list, tuple, set)):
                blocked = list(cinfo.get(k) or [])
                break
        # Also accept single name fields
        if not blocked:
            for k in ["night_last_day_doctor", "last_night_doctor"]:
                v = cinfo.get(k)
                if isinstance(v, str) and v.strip():
                    blocked = [v.strip()]
                    break
        if first_day and blocked:
            for doc in blocked:
                unav_map[norm_name(doc)][first_day].add("Any")

        # Nights history to enforce night spacing across month boundaries
        pre_from_gui = {}
        for k in ["recent_nights_by_doc", "prev_nights_by_doc", "night_history_by_doc"]:
            if isinstance(cinfo.get(k), dict):
                pre_from_gui = _normalize_dates_map(cinfo.get(k) or {})
                break

        merged_prev = defaultdict(list)
        for doc, ds in (prev_nights_by_doc or {}).items():
            merged_prev[norm_name(doc)].extend(list(ds or []))
        for doc, ds in (pre_from_gui or {}).items():
            merged_prev[norm_name(doc)].extend(list(ds or []))

        # Build slots for the month with carryover-aware night filtering
        slots_m = slots_for_month(cfg, days_m, unav_map, pre_nights_by_doc=dict(merged_prev))
        ...
        try:
            ...
            pass
        # Update carryover nights for next month in the SAME template (if any)
        if min_gap and min_gap > 1:
            last_k = min_gap - 1
            # collect last_k dates of this month
            dates_sorted = sorted([d.date for d in days_m])
            tail_dates = set(dates_sorted[-last_k:]) if dates_sorted else set()
            prev_nights_by_doc = defaultdict(list)
            for drow in days_m:
                if drow.date not in tail_dates:
                    continue
                sid = f"{drow.date}-J"
                doc = assignment_m.get(sid)
                if doc:
                    prev_nights_by_doc[norm_name(doc)].append(drow.date)
...
def extract_carryover_from_output_xlsx(
    ...
    '''Extract carryover information from a previously generated output.

    Returns a JSON-serializable dict with:
      - source_last_date (YYYY-MM-DD)
      - night_last_day_doctor
      - blocked_day1_doctors (list)
      - recent_nights_by_doc: {doctor: [YYYY-MM-DD, ...]} for the last (min_gap-1) days

    This is meant to be called by Streamlit to prepare `carryover_by_month`.
    '''
    p = Path(output_xlsx)
    wb = openpyxl.load_workbook(p)
    ws = wb[sheet_name] if sheet_name and sheet_name in wb.sheetnames else wb.active

    # find date rows in column A
    date_rows = []
    for r in range(2, ws.max_row + 1):
        v = ws.cell(row=r, column=1).value
        if isinstance(v, (dt.datetime, dt.date)):
            d = v.date() if isinstance(v, dt.datetime) else v
            date_rows.append((d, r))
    if not date_rows:
        return {}

    date_rows.sort(key=lambda x: x[0])
    last_date, last_row = date_rows[-1]

    # night doctor on last day
    col = str(night_col_letter).strip().upper() or "J"
    night_val = ws[f"{col}{last_row}"].value
    night_doc = norm_name(night_val) if night_val not in (None, "") else None

    # last (min_gap-1) nights
    k = max(0, int(min_gap) - 1)
    tail = date_rows[-k:] if k else date_rows[-1:]
    recent = {}
    for d, r in tail:
        v = ws[f"{col}{r}"].value
        if v is None or str(v).strip() == "":
            continue
        doc = norm_name(v)
        recent.setdefault(doc, []).append(d.isoformat())

    out = {
        "source_last_date": last_date.isoformat(),
        "night_last_day_doctor": night_doc,
        "blocked_day1_doctors": [night_doc] if night_doc else [],
        "recent_nights_by_doc": recent,
    }
    return out
...
def generate_schedule(
    ...
    **kwargs,
    ...
    slots, assignment, stats = solve_across_months(cfg, days, unav_map, carryover_by_month=carryover_by_month)
